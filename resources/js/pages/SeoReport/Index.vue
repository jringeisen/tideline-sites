...
async function submit(): Promise<void> {
    ...
    try {
        ...
        if (response.status === 429) {
            formError.value =
                'You have made a few requests already. Please wait a minute and try again.';
            phase.value = 'idle';
        ...
        formError.value =
            'Something went wrong starting your report. Please try again.';
        phase.value = 'idle';
    }
}
...
async function poll(): Promise<void> {
    ...
    try {
        ...
        if (body.status === 'failed') {
            failedMessage.value =
                body.message ?? 'We could not generate a report for that site.';
            phase.value = 'failed';
...
async function unlock(): Promise<void> {
    ...
    try {
        ...
        emailError.value =
            'Something went wrong unlocking your report. Please try again.';
        phase.value = 'teaser';
    }
}
...
<template>
    ...
    >
        ...
        <div
            class="mx-auto max-w-7xl px-6 pt-36 pb-16 sm:pt-44 lg:px-8 lg:pt-52"
        >
            <div class="max-w-2xl">
                <span
                ...
                >
                    Your free SEO
                    <span class="text-[var(--color-red)]">report.</span>
                </h1>
                <p class="mt-5 max-w-xl text-lg leading-relaxed text-white/85">
                    Enter your website and industry. We'll scan your site and
                    build a plain-English action plan — what's working, what's
                    not, and exactly what to fix first.
                </p>
            </div>
        </div>
    ...
    <section class="bg-[var(--color-cream)] py-16 sm:py-24">
        <div class="mx-auto max-w-3xl px-6 lg:px-8">
            ...
            >
                ...
                >
                    <p class="text-xs text-slate-500">
                        No credit card. No sales pitch. Results in under a
                        minute.
                    </p>
                    <button
                        type="submit"
            ...
            <div
                ...
                class="rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-[var(--color-sand-300)]/60"
            >
                <div
                    class="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-[var(--color-sand-200)] border-t-[var(--color-red)]"
                />
                <h2
                    class="mt-6 font-serif text-2xl text-[var(--color-navy-deep)]"
                >
                    Scanning your website…
                </h2>
                <p class="mt-2 text-sm text-slate-500">
                    We're reading your pages and building your action plan. This
                    usually takes 20–40 seconds.
                </p>
                <div class="mt-8 space-y-3">
                    <div
                        class="h-4 animate-pulse rounded-full bg-[var(--color-sand-200)]"
                    />
                    <div
                        class="h-4 w-5/6 animate-pulse rounded-full bg-[var(--color-sand-200)]"
                    />
                    <div
                        class="h-4 w-2/3 animate-pulse rounded-full bg-[var(--color-sand-200)]"
                    />
                </div>
            </div>
            ...
            <div
                ...
                class="rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-[var(--color-sand-300)]/60"
            >
                <h2 class="font-serif text-2xl text-[var(--color-navy-deep)]">
                    We hit a snag
                </h2>
            ...
            <div v-else class="space-y-8">
                ...
                >
                    ...
                    >
                        <div class="relative h-32 w-32 flex-none">
                            ...
                            >
                                <span
                                    class="font-serif text-4xl font-bold text-[var(--color-navy-deep)]"
                                    >{{ score }}</span
                                >
                                <span
                        ...
                        <div class="text-center sm:text-left">
                            <h2
                                class="font-serif text-2xl text-[var(--color-navy-deep)]"
                            >
                                Your SEO score
                            </h2>
                            <p
                ...
                <template v-if="phase === 'revealed' && report">
                    ...
                    >
                        <h3
                            class="font-serif text-xl text-[var(--color-navy-deep)]"
                        >
                            {{ section.title }}
                        </h3>
                        <ul class="mt-5 space-y-5">
                            <li
                                ...
                                :key="item.title"
                                class="border-l-2 border-[var(--color-sand-300)] pl-4"
                            >
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span
                                        ...
                                        class="rounded-full bg-[var(--color-sand-100)] px-2 py-0.5 text-xs text-slate-600"
                                    >
                    ...
                    <div
                        class="rounded-3xl bg-[var(--color-emerald-900)] p-6 text-white sm:p-8"
                    >
                        <h3 class="font-serif text-xl">
                            {{ report.google_business_profile.headline }}
                        </h3>
                        <ul class="mt-5 space-y-4 text-sm">
                            <li
                                v-for="step in report.google_business_profile
                                    .steps"
                                :key="step.title"
                            >
                    ...
                    <div
                        v-if="report.industry_tips.length"
                        class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-[var(--color-sand-300)]/60 sm:p-8"
                    >
                        <h3
                            class="font-serif text-xl text-[var(--color-navy-deep)]"
                        ...
                        <ul class="mt-5 space-y-4">
                            <li
                                v-for="tip in report.industry_tips"
                                :key="tip.title"
                            >
                                <p
                                    class="font-semibold text-[var(--color-ink)]"
                                ...
                                <p
                                    class="mt-0.5 text-sm leading-relaxed text-slate-600"
                                >
                                    {{ tip.detail }}
                                </p>
                            </li>
                ...
                <template v-else>
                    ...
                    >
                        ...
                        <div class="relative mt-4">
                            <div
                                class="space-y-3 blur-sm select-none"
                                aria-hidden="true"
                            >
                                <div
                                    v-for="n in Math.min(lockedCount, 4)"
                                    :key="n"
                                    class="rounded-xl bg-[var(--color-sand-100)] p-4"
                                >
                                    <div
                                        class="h-3 w-1/2 rounded-full bg-[var(--color-sand-300)]"
                                    />
                                    <div
                                        class="mt-2 h-3 w-3/4 rounded-full bg-[var(--color-sand-200)]"
                                    />
                                </div>
                            </div>
                            ...
                            <!-- Email gate -->
                            <div
                                class="mt-6 rounded-2xl border border-[var(--color-emerald-600)]/30 bg-[var(--color-emerald-900)] p-6 text-white sm:p-8"
                            >
                                <h4 class="font-serif text-2xl">
                                    Unlock your full report
                                </h4>
                                <p class="mt-2 text-sm text-white/80">
                                    Enter your email to reveal all
                                    {{ lockedCount }} remaining recommendations,
                                    your Google Business Profile checklist, and
                                ...
                                <p class="mt-3 text-xs text-white/60">
                                    We'll never share your email. Unsubscribe
                                    anytime.
                                </p>
                            </div>
                        </div>
