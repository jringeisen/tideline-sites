...
type Post = PostCard & {
    ...
    author: { name: string | null; avatar_url: string | null; bio: string | null };
};
...
const canonical = computed<string>(
    () => ((page.props.meta as { canonical?: string } | undefined)?.canonical ?? page.url),
);
...
<template>
    <article class="bg-[var(--color-cream)]">
            ...
            <div class="mx-auto max-w-3xl px-6 pt-32 pb-20 sm:pt-40 lg:px-8">
                <nav aria-label="Breadcrumb" class="text-sm text-white/60">
                    <ol class="flex flex-wrap items-center gap-2">
                        ...
                        <li><Link href="/blog" class="hover:text-white">Blog</Link></li>
                        <template v-if="post.category">
                            <li aria-hidden="true">›</li>
                            <li>
                                <Link :href="`/blog/category/${post.category.slug}`" class="hover:text-white">{{ post.category.name }}</Link>
                            </li>
                        </template>
                    </ol>
                </nav>
                <h1 class="mt-6 font-serif text-4xl leading-tight font-bold tracking-tight sm:text-5xl">{{ post.title }}</h1>
                <div class="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-white/75">
                    ...
                    <time :datetime="post.published_at_iso ?? undefined">{{ post.published_at_full }}</time>
        ...
        <div class="mx-auto max-w-3xl px-6 py-16 lg:px-8">
            <div
                class="prose prose-slate max-w-none prose-headings:font-serif prose-headings:text-[var(--color-deep-teal)] prose-a:text-[var(--color-emerald-700)] prose-img:rounded-xl dark:prose-invert"
                v-html="post.content"
            />
            ...
            <section aria-label="Share this post" class="mt-12 flex flex-wrap items-center gap-3 border-y border-[var(--color-sand-300)] py-6">
                <span class="text-xs font-semibold tracking-[0.18em] text-[var(--color-red)] uppercase">Share</span>
                <a
            ...
            >
                <img v-if="post.author.avatar_url" :src="post.author.avatar_url" alt="" class="h-14 w-14 rounded-full object-cover" />
                <div>
        ...
        <section v-if="related.length" class="bg-[var(--color-sand-100)] py-16">
            <div class="mx-auto max-w-5xl px-6 lg:px-8">
                <p class="text-xs font-semibold tracking-[0.18em] text-[var(--color-red)] uppercase">Keep reading</p>
                <h2 class="mt-3 font-serif text-3xl font-bold tracking-tight text-[var(--color-deep-teal)] uppercase">More from the blog</h2>
                <div class="mt-8 grid gap-6 sm:grid-cols-3">
                    <BlogPostCard v-for="item in related" :key="item.slug" :post="item" />
                </div>
            </div>
        </section>
    </article>
