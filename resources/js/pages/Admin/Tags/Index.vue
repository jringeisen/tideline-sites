...
defineOptions({
    layout: {
        breadcrumbs: [
            { title: 'Tags', href: tagsRoutes.index().url },
        ],
    },
});
            ...
            <tbody class="divide-y divide-[var(--color-sand-300)]/60 dark:divide-white/10">
                ...
                >
                    <td class="px-5 py-4 font-medium text-[var(--color-deep-teal)]">
                        <Link :href="tagsRoutes.edit(tag.id).url" class="hover:underline">{{ tag.name }}</Link>
                    </td>
                    <td class="px-5 py-4 text-slate-600 dark:text-white/70">{{ tag.slug }}</td>
                    <td class="px-5 py-4 text-slate-600 dark:text-white/70">{{ tag.posts_count }}</td>
                ...
                <tr v-if="tags.length === 0">
                    <td colspan="4" class="px-5 py-12 text-center text-slate-500 dark:text-white/60">
                        <p class="font-serif text-xl text-[var(--color-deep-teal)]">No tags yet.</p>
                        <p class="mt-1 text-sm">Tags help readers explore related posts.</p>
                    </td>
                </tr>
            </tbody>
        </table>
