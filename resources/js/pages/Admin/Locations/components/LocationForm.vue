...
const segments = ref<Segment[]>(
    (props.initialSegments ?? []).map((s) => ({ ...s })),
);
const faqs = ref<Faq[]>((props.initialFaqs ?? []).map((f) => ({ ...f })));
const nearbyIds = ref<number[]>([...(props.initialNearbyIds ?? [])]);
const serviceIds = ref<number[]>([...(props.initialServiceIds ?? [])]);
...
const addSegment = () => segments.value.push({ title: '', body: '' });
const removeSegment = (i: number) => segments.value.splice(i, 1);
...
const toggleService = (id: number) => {
    serviceIds.value = serviceIds.value.includes(id)
        ? serviceIds.value.filter((x) => x !== id)
        : [...serviceIds.value, id];
};

const toggleNearby = (id: number) => {
    nearbyIds.value = nearbyIds.value.includes(id)
        ? nearbyIds.value.filter((x) => x !== id)
        : [...nearbyIds.value, id];
};
...
const transform = (data: Record<string, unknown>) => ({
    ...
    segments: segments.value.filter((s) => s.title.trim() || s.body.trim()),
    faqs: faqs.value.filter((f) => f.question.trim() || f.answer.trim()),
    nearby: nearbyIds.value,
...
const chipBase = 'rounded-full px-3 py-1 text-xs ring-1 transition';
const chipOn =
...
<template>
    ...
    >
        <section :class="cardClass">
            ...
            <div class="grid gap-2 sm:grid-cols-2 sm:gap-4">
                ...
                <div class="grid gap-2">
                    ...
                    <Input
                        id="display_name"
                        name="display_name"
                        :default-value="initial?.display_name ?? ''"
                        required
                        maxlength="200"
                    />
            ...
            <div class="grid gap-2">
                <Label for="why_local">Why local</Label>
                <textarea
                    id="why_local"
                    name="why_local"
                    rows="3"
                    maxlength="2000"
                    :default-value="initial?.why_local ?? ''"
                    :class="inputClass"
                />
                <InputError :message="errors.why_local" />
            </div>
            ...
            <div class="grid gap-2">
                ...
                <TipTapEditor
                    v-model="body"
                    placeholder="Write genuinely local, in-depth content…"
                />
        ...
        <section :class="cardClass">
            <div class="flex items-center justify-between">
                ...
                <Button type="button" variant="secondary" @click="addSegment">
                    Add segment
                </Button>
            </div>
            ...
            >
                <Input v-model="segment.title" placeholder="Segment title" />
                <textarea
                    v-model="segment.body"
                    rows="2"
                ...
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="[
                                chipBase,
                                nearbyIds.includes(location.id)
                                    ? chipOn
                                    : chipOff,
                            ]"
                            @click="toggleNearby(location.id)"
                        >
                            {{ location.name }}
                        </button>
                        <span
