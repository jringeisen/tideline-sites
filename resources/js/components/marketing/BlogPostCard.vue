...
<template>
    <article class="group flex flex-col rounded-2xl bg-white p-7 ring-1 ring-[var(--color-sand-300)]/60 transition hover:ring-[var(--color-red)]/30 dark:bg-white/[0.04] dark:ring-white/10">
        ...
        <h3 class="mt-3 font-serif text-2xl leading-tight tracking-tight text-[var(--color-deep-teal)]">
            <Link :href="`/blog/${post.slug}`" class="hover:underline">{{ post.title }}</Link>
        </h3>

        <p v-if="post.excerpt" class="mt-3 line-clamp-3 text-sm leading-relaxed text-slate-600">{{ post.excerpt }}</p>

        <div class="mt-6 flex items-center justify-between text-xs text-slate-500">
            <div class="flex items-center gap-2">
                <img v-if="post.author.avatar_url" :src="post.author.avatar_url" alt="" class="h-6 w-6 rounded-full object-cover" />
                <span>{{ post.author.name ?? 'All American Web Design' }}</span>
            </div>
            <div class="flex items-center gap-3">
                <time :datetime="post.published_at_iso ?? undefined">{{ post.published_at }}</time>
